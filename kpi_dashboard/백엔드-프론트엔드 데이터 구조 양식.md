...
### 배치 통계 API (`POST /api/kpi/statistics/batch`)

대시보드 초기 렌더링 등 여러 KPI를 동시에 조회합니다. 응답은 각 KPI 타입 키로 그룹화된 객체를 반환합니다.
...
{
  "data": {
    "availability": [{ "timestamp": "...", "entity_id": "...", "value": 99.0 }],
    "rrc": [{ "timestamp": "...", "entity_id": "...", "value": 98.5 }]
  },
  "source": "proxy-mock"
}
```
...
## 3. 종합 분석 리포트 데이터 (`/api/analysis/results/*`, `/api/reports/summary`)
...
### 분석 결과 저장 모델 (`/api/analysis/results`)

백엔드는 MongoDB(Motor) 기반으로 분석 결과를 저장합니다. 주요 엔드포인트(컬렉션 리소스):

- `POST /api/analysis/results` → 분석 결과 생성
- `GET /api/analysis/results` → 목록 조회(페이지네이션/필터)
- `GET /api/analysis/results/{id}` → 단건 상세 조회
- `PUT /api/analysis/results/{id}` → 단건 수정
- `DELETE /api/analysis/results/{id}` → 단건 삭제

요청 바디 예시 (`POST /api/analysis/results`):
```json
{
  "analysisType": "llm_analysis",
  "analysisDate": "2025-08-07T23:59:59+09:00",
  "neId": "ALL",
  "cellId": "ALL",
  "status": "success",
  ...
  "results": [],
  "stats": [
    { "period": "N-1", "kpi_name": "Random_access_preamble_count", "avg": 123.45 },
    { "period": "N",   "kpi_name": "Random_access_preamble_count", "avg": 120.10 }
  ],
  "analysis": {
    "executive_summary": "...",
    "diagnostic_findings": [
      { "primary_hypothesis": "...", "supporting_evidence": "...", "confounding_factors_assessment": "..." }
    ],
    "recommended_actions": [
      { "priority": "P1", "action": "...", "details": "..." }
    ],
    ...
    "source_metadata": {
      "db_config": { "host": "...", "port": 5432, "user": "postgres", "dbname": "..." },
      "table": "summary",
  ...
  "request_params": {
    "db": { "host": "...", "port": 5432, "user": "postgres", "dbname": "..." },
    "table": "summary",
    "columns": { "time": "datetime", "peg_name": "peg_name", "value": "value" },
    "time_ranges": {
      "n_minus_1": { "start": "2025-08-06T00:00:00+09:00", "end": "2025-08-06T23:59:59+09:00" },
      "n": { "start": "2025-08-07T00:00:00+09:00", "end": "2025-08-07T23:59:59+09:00" }
    },
    "filters": { "ne": ["nvgnb#10000"], "cellid": ["2010","2011"] },
    "preference": "...",
    "selected_pegs": ["..."],
    "peg_definitions": { "telus_RACH_Success": "Random_access_preamble_count/Random_access_response*100" }
  }
}
```
응답 예시:
```json
{
  ...
  "data": {
    ...
    "status": "success",
    "results": [],
    "stats": [ { "period": "N-1", "kpi_name": "...", "avg": 123.45 } ],
    "report_path": "/path/to/report.html",
    "analysis": { "executive_summary": "..." },
    "metadata": { "created_at": "...", "updated_at": "...", "analysis_type": "llm_analysis", "version": "1.0" }
  }
}
```
