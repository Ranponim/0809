...
프론트엔드에서 가장 고도화된 컴포넌트로, LLM 분석 결과를 체계적으로 관리하고 시각화합니다.

#### 주요 컴포넌트 구조:
- **ResultsList.jsx**: 분석 결과 목록 표시, 필터링, 정렬, 삭제 기능
- **ResultDetail.jsx**: 상세 보기 모달, 다양한 알고리즘 분석 결과 표시
- **ResultFilter.jsx**: 필터링 컴포넌트, NE/Cell ID/날짜 범위 필터링

#### 고급 분석 기능:
- **마할라노비스 거리 분석**: 다차원 이상치 감지 및 심층 분석
- **PEG 비교 분석**: N-1/N 기간별 성능 비교 및 변화율 계산
- **Mann-Whitney U Test**: 그룹 간 차이 통계 검정
- **Kolmogorov-Smirnov Test**: 분포 차이 검정
- **Choi 알고리즘 판정 결과**: 품질 저하 판정 (준비 단계)

#### 주요 특징:
- **메모리 최적화**: 청크 단위 데이터 처리 (50개씩)
- **키보드 단축키**: F11(전체화면), Escape(전체화면 해제)
- **실시간 차트**: PEG별 N-1/N 성능 비교 차트
